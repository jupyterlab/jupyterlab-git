...
@fixture
async def http_server(http_server_port, jp_web_app):
    """Override to run in the test's event loop (pytest-asyncio 1.x compatibility)."""
...
async def http_server_client(http_server):
    """Override to run in the test's event loop (pytest-asyncio 1.x compatibility)."""
    from pytest_jupyter.pytest_tornasync import AsyncHTTPServerClient
