"""
This is a Module where actual Git Commands are written & Executed &  the result
...
class Git:
    ...
    def log_1(self, selected_hash, current_path):
        """
        Function used to execute the second git log command (used to get
        ...
        if (p.returncode == 0):
            ...
            if (length > 1):
                ...
                for num in range(1, int(length / 2)):
                    ...
                    length = len(words)
                    result.append({
                        "modified_file_path": line_info[2],
    ...
    def branch(self, current_path):
        ...
        if (p.returncode == 0):
            ...
            local or remote, Should have btter ways
            '''
    ...
    def showtoplevel(self, current_path):
        """
        Function used to execute git --show-toplevel command & send back the
        result.
        """
        p = Popen(
    ...
    def showprefix(self, current_path):
        """
        Function used to execute git --show-prefix command & send back the
        result.
        """
        p = Popen(
    ...
    def add(self, filename, top_repo_path):
        """
        Function used to execute git add<filename> command & send back the
        result.
        """
    ...
    def add_all_untracked(self, top_repo_path):
        """
        Function used to execute git add_all_untracked command & send back the
        result.
        """
        e = 'echo "a\n*\nq\n" | git add -i'
    ...
    def reset(self, filename, top_repo_path):
        """
        Function used to execute git reset <filename> command & send back the
        result.
        """
        my_output = subprocess.check_output(
    ...
    def checkout_new_branch(self, branchname, current_path):
        """
        Function used to execute git checkout <make-branch> command & send back
        the result.
        """
        p = Popen(
    ...
    def checkout_branch(self, branchname, current_path):
        """
        Function used to execute git checkout <branch-name> command & send back
        the result.
        """
        p = Popen(
    ...
    def checkout(self, filename, top_repo_path):
        """
        Function used to execute git checkout for the filename & send back the
        result.
        """
        my_output = subprocess.check_output(
    ...
    def checkout_all(self, top_repo_path):
        """
        Function used to actually execute git checkout command & send back the
        result.
        """
        my_output = subprocess.check_output(
