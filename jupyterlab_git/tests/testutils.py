...
def assert_http_error(error, expected_code, expected_message=None):
    ...
    if isinstance(e, tornado.web.HTTPError):
        assert expected_code == e.status_code, f"Expected status code {expected_code} != {e.status_code}"
        if expected_message is not None:
            assert expected_message == str(e), f"Expected error message '{expected_message}' != '{str(e)}'"
        ...
        if expected_message:
            ...
            assert expected_message == message, f"Expected error message '{expected_message}' != '{message}'"
