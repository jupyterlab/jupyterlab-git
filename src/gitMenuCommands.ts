...
export function addCommands(app: JupyterFrontEnd, services: ServiceManager) {
  ...
  const tracker = new WidgetTracker<Terminal>({ namespace });
  let gitApi = new Git();
  ...
  commands.addCommand(CommandIDs.gitTerminal, {
    ...
    execute: async args => {
      ...
      let session = name
        ? await services.terminals.connectTo(name)
        : await services.terminals.startNew();
      let terminal = new Terminal(session);
  ...
  commands.addCommand(CommandIDs.gitTerminalCommand, {
    ...
    execute: async args => {
      ...
      let session = await services.terminals.startNew();
      let terminal = new Terminal(session);
      terminal.title.closable = true;
      terminal.title.label = '...';
      app.shell.add(terminal, 'main');
