...
namespace GitSessionNode {
  ...
  /** Refresh widget, update everything */
    ...
    try {
      ...
      if (api_result.code === 0) {
        ...
        if (api_status.code === 0) {
          ...
          if (Changes === 0) {
          // No uncommitted changes, enable switch branch button 
        ...
        this.setState(
          {
            ...
            top_repo_path: (api_showtoplevel as GitShowTopLevelResult).top_repo_path, 
            show:true, 
            branches: (api_branch as GitBranchResult).branches, 
            current_branch: current_branch, 
            disable_switch_branch: disable_switch_branch, 
            pull_enable:pull_enable, 
            push_enable:push_enable,
            past_commits: past_commits, 
            in_new_repo: in_new_repo, 
  ...
  render() {
    return(
      <div className='jp-Git-container'>
        <PathHeader 
          current_fb_path={this.state.current_fb_path} 
          top_repo_path={this.state.top_repo_path} 
        ...
        <ToggleDisplay show={this.state.show}>
          <BranchHeader 
            ...
            top_repo_path={this.state.top_repo_path} 
            refresh={this.refresh} 
            current_branch={this.state.current_branch} 
            data={this.state.branches} 
            disabled={this.state.disable_switch_branch}
          />
          <PastCommits 
            current_fb_path={this.state.current_fb_path} 
            top_repo_path={this.state.top_repo_path} 
            past_commits={this.state.past_commits} 
            in_new_repo={this.state.in_new_repo}
            show_index={this.state.show_index}
            staged_files={this.state.staged_files} 
            unstaged_files={this.state.unstaged_files} 
            untracked_files={this.state.untracked_files} 
            app={this.props.app} 
            refresh={this.refresh} 
            show_current_work={this.show_current_work} 
