...
export class ResetDeleteSingleCommit extends React.Component<IResetDeleteProps, IResetDeleteState> {
  constructor(props) {
    super(props);
    this.state = {
      message: '',
  ...
  handleResetDelete = () => {
    ...
    if (this.props.action === 'reset') {
      gitApi.resetToCommit(this.state.message, this.props.path, this.props.commitId)
    } else {
  ...
  render() {
    return (
      <div>
        ...
        />
        <button className={classes(Button, CancelButton)}
          onClick={this.onCancel}
        >
          Cancel
        </button>
        <button 
          className={this.state.resetDeleteDisabled 
